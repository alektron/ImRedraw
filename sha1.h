...
//Source: https://github.com/983/SHA1
//alektron: Fixed some warnings. Not sure about that last one
...
public:
    ...
    const sha1& print_hex(
        ...
        for (int n = 0; n < 5; n++){
            for (int j = 7; j >= 0; j--){
                hex[k++] = alphabet[(state[n] >> j * 4) & 0xf];
            }
        }
    ...
    const sha1& print_base64(char *base64, bool zero_terminate = true) const {
        ...
        for (int n = 0; n < 7; n++){
            uint32_t x = triples[n];
            base64[n*4 + 0] = table[(x >> 3*6) % 64];
            base64[n*4 + 1] = table[(x >> 2*6) % 64];
            base64[n*4 + 2] = table[(x >> 1*6) % 64];
            base64[n*4 + 3] = table[(x >> 0*6) % 64];
        }
